...
var addresss = '';
var ignition = '';
var balance = { trnd: 0, xtrnd: 0, dai_univ2: 0, eth_univ2: 0 };
  ...
  put(
    ...
    function(data) {
      if (MODE == 'PROD') {
        ...
        data.responseText = data.responseText.replace('{balance_trnd}','0');
        data.responseText = data.responseText.replace('{balance_xtrnd}','0');
        data.responseText = data.responseText.replace('{fueldepo_xtrnd}','0');
      ...
      if (data.result == 'OK') {
        ...
        setTimeout(function () {
          $('#monitoring-tokens .token-logo').each(function(index, el) {
            ...
            $(el).attr('src',source).css('opacity','1')
          });
        }, 500);
      }
      if (data.result == 'Err') {
  ...
  if (monitoringTokensInterval === undefined && MODE == 'PROD') {
    ...
    monitoringTokensInterval = setInterval(function(){
      ...
      put(
        ...
        '/workspace/monitoring/tokens',
        // payload
        { address: address, latest_hash: $('#tokens-new > div:first-child').attr('id') },
        // done
        function(data) {
          if (data.result == 'OK') {
            if (data.message && data.message.length) $('#tokens-new').prepend(data.message);
...
const placeholderImage = (el) => {
  $(el).attr('src','/img/trnd-eye.png').css('opacity','0.25');
}
